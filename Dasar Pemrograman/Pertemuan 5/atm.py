...
def tranfer_uang(uang,id_user):
    index1 = cek_user(user_id)
    index2 = cek_rekening(id_user)
    if index1 >= 0:
        if user[index1]['saldo'] >= int(uang):
            user[index1]['saldo'] =user[index1]['saldo'] - int(uang)
            pelanggan[index2]['saldo'] =pelanggan[index2]['saldo'] + int(uang)
            print("Anda berhasil mentransfer uang Rp."+str(uang)+" ke Rekening "+ id_user)
